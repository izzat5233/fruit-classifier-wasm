// Credits to `Armchair-Software` for this utility header
// Repository: https://github.com/Armchair-Software/emscripten-browser-file
    ...
    EM_JS_INLINE(void, upload, (char const *accept_types, upload_handler callback, void *callback_data), {
        ...
        globalThis["open_file"] = function(e)
        {
            ...
            file_reader.onload = (event) => {
                ...
                Module["ccall"]('upload_file_return',
                                'number',['string', 'string', 'number', 'number', 'number', 'number'],[event.target.filename, event.target.mime_type, data_on_heap.byteOffset, uint8Arr.length, callback, callback_data]);
    ...
    EM_JS_INLINE(void, download, (char const *filename, char const *mime_type, void const *buffer, size_t buffer_size),
                 {
                     ...
                     a.href = URL.createObjectURL(new Blob([new Uint8Array(Module["HEAPU8"].buffer, buffer,
                                                                           buffer_size)], {type: UTF8ToString(
                             mime_type)}));
                     a.click();
                 });
#pragma GCC diagnostic pop
    ...
    namespace {
        ...
        EMSCRIPTEN_KEEPALIVE inline int
        upload_file_return(char const *filename, char const *mime_type, char *buffer, size_t buffer_size,
